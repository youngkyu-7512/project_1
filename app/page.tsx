...
type DownloadOutcome =
  ...
  | { kind: "folder_invalid"; reason: string }
  | { kind: "bad_request" };
...
async function streamDownload(
  params: { url: string; videoFormatId: string; folder: string },
...
export default function Home() {
  ...
  useEffect(() => {
    fetch("/api/video/folder/defaults")
      .then((r) => r.json())
      .then((data: { defaultFolder: string; recentFolders: string[] }) => {
        setDefaultFolder(data.defaultFolder);
        setFolder(data.defaultFolder);
  ...
  const handleStartDownload = async () => {
    ...
    const outcome = await streamDownload({ url, videoFormatId: selectedQualityId, folder }, setProgress);

    if (outcome.kind === "folder_invalid") {
      setFolderError(outcome.reason);
      setStage("ready");
      return;
    }
    if (outcome.kind === "success" || outcome.kind === "already_exists") {
      setRecentFolders((prev) => [folder, ...prev.filter((p) => p !== folder)].slice(0, 5));
    }
  ...
  const handleOpenFolder = async (path: string) => {
    await fetch("/api/video/folder/open", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ folder: path }),
    });
    setOpenedToast(true);
    setTimeout(() => setOpenedToast(false), 2600);
  };
    ...
    <div className="flex flex-1 justify-center bg-zinc-50 py-10 dark:bg-black">
      <div className="flex w-full max-w-2xl flex-col gap-4 px-4">
        ...
        <p className="text-sm text-muted-foreground">
          동영상이 있는 웹 페이지 주소를 붙여넣으면, 받을 화질을 고르고 내 PC 폴더에 파일로 저장합니다.
        </p>
        ...
        <Card>
          ...
          <CardContent>
            ...
            {extractFailure && (
              <Alert
                variant={extractFailure.kind === "unsupported_url" ? "default" : "default"}
                className="mt-3"
              >
                <RiErrorWarningLine />
        ...
        {/* 3. 저장 폴더 */}
        {video && (stage === "ready" || stage === "downloading") && (
          <Card>
            <CardHeader>
              <CardTitle className="flex items-center gap-2">
                <StepBadge n={3} />
            ...
            <CardContent>
              <Field>
                <FieldLabel>이 폴더에 저장합니다</FieldLabel>
                <Input
                  value={folder}
                  onChange={(e) => {
              ...
              {recentFolders.length > 0 && (
                <Field>
                  <FieldLabel>최근에 저장한 폴더</FieldLabel>
                  <div className="flex flex-wrap gap-1.5">
                    {recentFolders.map((path) => (
                      <button
                        ...
                        disabled={stage === "downloading"}
                        className={cn(
                          "max-w-full truncate rounded-full border px-3 py-1 font-mono text-xs text-muted-foreground hover:bg-muted hover:text-foreground disabled:pointer-events-none disabled:opacity-50",
                          path === folder && "border-primary bg-primary/10 text-primary",
                        )}
                      >
                        {path}
                      </button>
              ...
              {folderError && (
                <Alert variant="destructive">
                  <RiErrorWarningLine />
                  <AlertTitle>폴더를 찾을 수 없습니다</AlertTitle>
                  <AlertDescription>{folderError} 다운로드는 시작하지 않았습니다.</AlertDescription>
                </Alert>
              )}
              ...
              <Button
                size="lg"
                className="w-full"
                disabled={selectedQualityId === "" || !folder.trim() || stage === "downloading"}
          ...
          <Card>
            <CardContent>
              <DownloadResult
                ...
                onReset={resetAll}
                onRetry={() => void handleStartDownload()}
...
function extractFailureTitle(failure: ExtractionFailure): string {
  switch (failure.kind) {
    ...
    case "missing_tools":
      return failure.tool === "ffmpeg" ? "ffmpeg을 찾을 수 없습니다" : "yt-dlp를 찾을 수 없습니다";
    default:
      return "추출하지 못했습니다";
  }
}
...
function extractFailureMessage(failure: ExtractionFailure): string {
  switch (failure.kind) {
    ...
    case "missing_tools":
      return failure.tool === "ffmpeg"
        ? "영상과 소리를 하나의 파일로 합치려면 ffmpeg이 필요합니다. winget install Gyan.FFmpeg 으로 설치한 뒤 다시 시도하세요."
        : "동영상을 찾으려면 yt-dlp가 필요합니다. winget install yt-dlp.yt-dlp 로 설치한 뒤 다시 시도하세요.";
    default:
        ...
        <div className="flex items-center gap-2.5">
          <span
            className={cn(
              ...
              outcome.kind === "success" ? "bg-primary/15 text-primary" : "bg-muted text-muted-foreground",
            )}
          >
          ...
          <div>
            <div className="text-[15px] font-semibold">
              {outcome.kind === "success" ? "저장을 마쳤습니다" : "이미 같은 파일이 있습니다"}
            </div>
            {outcome.kind === "already_exists" && (
              <div className="text-sm text-muted-foreground">다시 내려받지 않았습니다.</div>
            )}
          </div>
        </div>
        <div className="grid gap-2 rounded-md border bg-muted p-3">
          <Row k="폴더" v={outcome.filePath.slice(0, outcome.filePath.length - outcome.fileName.length - 1)} />
          <Row k="파일 이름" v={outcome.fileName} />
        </div>
        <div className="flex flex-wrap gap-2">
          <Button variant="outline" onClick={() => onOpenFolder(outcome.filePath.slice(0, outcome.filePath.length - outcome.fileName.length - 1))}>
            <RiFolderOpenLine />
            폴더 열기
          </Button>
...
function Row({ k, v }: { k: string; v: string }) {
  return (
    <div className="grid grid-cols-[62px_1fr] items-baseline gap-3">
      ...
      <span className="font-mono text-[13px] break-all">{v}</span>
    </div>
  );
}
